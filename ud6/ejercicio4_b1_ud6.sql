...
ALTER TABLE Usuario ADD CONSTRAINT ck_longitud_pass CHECK (CHAR_LENGTH(contrasenya) >=4 --LENGTH mejor que char_length
															  AND CHAR_LENGTH(contrasenya) <=8);
ALTER TABLE Usuario ADD CONSTRAINT ck_espacios_pass CHECK (POSITION(' ' IN contrasenya) = 0); --contrasenya NOT LIKE '% %'
ALTER TABLE Estacion ADD CONSTRAINT ck_empieza_e_cod_estacion CHECK (SUBSTRING(cod_estacion, 1, 1) = 'E');
